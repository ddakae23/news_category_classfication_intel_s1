...
# # print(list(resp))
# print(type(resp))
# soup = BeautifulSoup(resp.text, 'html.parser')    # BeautifelSoup - html을 받아서 text 문서로 받음
# # print(soup)
...
#     titles.append(re.compile('[^가-힣|a-z|A-Z]').sub(' ', title_tag.text))    # ^ - 처음부터, .sub - 앞의 것을 제외한 것을 빼고 ' '로 채워라.
# print(titles)
# print(len(titles))
...
for i in range(6):
    ...
    df_titles = pd.concat([df_titles, df_section_titles], axis='rows', ignore_index=True)       #  앞에꺼에 이어서 뒤에꺼를 붙여서 덮어 씌움.
